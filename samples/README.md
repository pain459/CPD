...
* Currencies: `USD`, `EUR`, `GBP` (FX-normalized in Transform)
...
| `sales_dirty.csv` | 25 total: 6 ok / 19 rejected | Validation matrix: every defect class at least once |
| `sales_edge.csv` | 12 | Parser hardening boundaries (see below) |
...
| `manifest.json` | — | Expected counts for automated tests |

## Dirty-file defect map (`sales_dirty.csv`)

Rejected (19): duplicate full row (O-2001 ×2) · conflicting duplicate `O-2015` (2 dates) · invalid dates (`bad-date`, month 13) · negative qty · zero qty · unknown SKU-999 · unknown store S-99 · empty price · unparseable price · unknown currency XYZ · unknown payment `bitcoin` · empty `order_id` · empty `product_name` · empty `customer_id` · extra column · short row · implausible qty 1,000,000.

Valid-by-design (6, parser must be lenient here): `O-2016` mixed date `09/06/2026` · `O-2017` uppercase `CARD` · `O-2018` lowercase `usd` · `O-2019` padded `  Bread  ` (trim) · `O-2020` trailing comma (empty extra field ignored) · `O-2023` clean control row.

## Edge file (`sales_edge.csv`, 12 rows)

Leap-day 2024-02-29 · zero price · 3-decimal price · unicode (`Café au lait`) · max-int qty · case variants (`Cash`, `APPLE`) · ISO datetime · padded fields · GBP row · repeating decimal · month-end date.
